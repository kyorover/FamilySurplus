...
本システムはAWS CDK（`hesokuri-backend-stack.ts`）によって定義された **6つ** のDynamoDBテーブルで構成されています。すべてのテーブルは `PAY_PER_REQUEST`（オンデマンドキャパシティ）に設定され、スタック削除時のポリシーは `DESTROY` に設定されています。
...
## 4. AccountsTable（アカウント・課金情報テーブル）
...
## 5. MonthlySummariesTable（月次サマリーテーブル）【新規追加】

### 5.1. インフラ定義
- **テーブル名参照**: 環境変数 `SUMMARIES_TABLE_NAME`
- **パーティションキー (PK)**: `householdId` (String型) ※Cognitoのsubと一致
- **ソートキー (SK)**: `month_id` (String型)

### 5.2. データスキーマ（項目属性）
TypeScriptの `MonthlySummary` インターフェースに基づき、月締め完了時の確定へそくり履歴が保存されます。

- `householdId` (string): パーティションキー
- `month_id` (string): ソートキー (YYYY-MM形式。例: 2026-03)
- `isConfirmed` (boolean): 月締めが完了し、へそくり額が確定したかを示すフラグ
- `confirmedHesokuriAmount` (number): 確定後のへそくり額（以後の過去月支出修正の影響を受けない不変の値）
- `confirmedAt` (string / 任意): 確定処理を行った日時（ISO 8601形式）
- `updatedAt` (string): 最終更新日時

---

## 6. SystemConfigTable（システム設定・統計データキャッシュテーブル）【新規追加】

### 6.1. インフラ定義
- **テーブル名参照**: 環境変数 `SYSTEM_CONFIG_TABLE_NAME`
- **パーティションキー (PK)**: `PK` (String型)
- **ソートキー (SK)**: `SK` (String型)

### 6.2. データスキーマ（項目属性）
EventBridgeから定期実行されるバックエンドのバッチ処理により取得された、外部の公的統計データ（物価指数等）がフロントエンド提供用のキャッシュとして保存されます。

- `PK` (string): パーティションキー (例: `CONFIG#STATISTICS`)
- `SK` (string): ソートキー (対象月。例: `MONTH#2026-04`)
- `data` (Object): 統計データの実体（`NationalStatistics`型）。`cpi`（消費者物価指数）や `averageExpenses`（単身・2人以上世帯の固定支出平均）を保持する。※収入（income等）に関するデータは一切含まれない。
- `createdAt` (string): キャッシュレコード作成日時