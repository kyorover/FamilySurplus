...
4. **【重要】各UI部品が持つ「内包されたマージン」を再計算し、画面全体で端（16px）が揃っているか物理的整合性を確認せよ。**
5. **【重要】変更検知（hasChanges/isDirty判定）に用いるStoreの同期ロジックを破壊していないか確認せよ。**
6. この一覧を辞書として保持し、今後の要求に対して正確な推論とコード生成を実施します。