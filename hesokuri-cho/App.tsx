...
export default function App() {
  ...
  const handleTabChange = (targetTab: typeof activeTab) => {
    ...
    if (activeTab === 'settings' && targetTab !== 'settings') {
      ...
      if (isChanged && pendingSettings) {
        Alert.alert('未保存の変更', '設定が変更されています。保存して移動しますか？', [
          { text: '破棄して移動', style: 'destructive', onPress: () => { setPendingSettings(null); setActiveTab(targetTab); } },
          { text: 'キャンセル', style: 'cancel' },
          { text: '保存して移動', onPress: async () => { await updateSettings(pendingSettings); setActiveTab(targetTab); } }
        ]);
        return;
      }
    }
  ...
  if (isLoading && (!settings || !monthlyBudget)) return (
    <View style={styles.centerContainer}><ActivityIndicator size="large" color="#007AFF" /><Text style={{ marginTop: 16 }}>読み込み中...</Text></View>
  );

  if (error && !settings) return (
    <View style={styles.centerContainer}>
      <Text style={{ color: '#FF3B30', marginBottom: 16, fontWeight: 'bold' }}>通信エラー</Text><Text style={{ textAlign: 'center', marginBottom: 24 }}>{error}</Text>
      <TouchableOpacity style={styles.primaryButton} onPress={() => { fetchSettings(); fetchMonthlyBudget(new Date().toISOString().slice(0, 7)); }}><Text style={styles.primaryButtonText}>再試行</Text></TouchableOpacity>
    </View>
  );
  ...
  if (!settings) return (
    <View style={styles.centerContainer}>
      ...
      }}><Text style={styles.primaryButtonText}>初期データを生成して開始</Text></TouchableOpacity>
    </View>
  );
    ...
    <SafeAreaView style={styles.container}>
      {!isSubScreen && (
        <View style={styles.header}>
          <Text style={styles.headerTitle}>{activeTab === 'dashboard' ? 'へそくりダッシュボード' : activeTab === 'input' ? '支出の記録' : '家計の設定'}</Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  primaryButton: { backgroundColor: '#007AFF', paddingHorizontal: 24, paddingVertical: 14, borderRadius: 8 },
  ...
  navItemMain: { flex: 1, alignItems: 'center', backgroundColor: '#007AFF', paddingVertical: 12, borderRadius: 24, marginHorizontal: 16 },
