...
interface Props {
  ...
  selectedItemIndex?: number | null; // 追加: 選択中のアイテムインデックス
}
...
export const IsometricGardenCanvas: React.FC<Props> = ({ placements = [], onPressTile, selectedItemIndex }) => {
      ...
      {renderList.map((node) => {
        ...
        if (node.type === 'floor') {
          ...
          return (
            ...
            >
              {/* ドラッグコンポーネントを廃止し、選択時のハイライト用スタイルを付与 */}
...
const styles = StyleSheet.create({
  ...
  selectedHighlight: {
    backgroundColor: 'rgba(255, 215, 0, 0.4)', // 選択中は薄いゴールドの背景でハイライト
    borderWidth: 2,
    borderColor: '#FFD700',
  }
});