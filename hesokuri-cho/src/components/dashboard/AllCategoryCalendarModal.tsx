...
export const AllCategoryCalendarModal: React.FC<AllCategoryCalendarModalProps> = ({ visible, categories, expenses, currentMonth, initialDate, onClose, onEditExpense, onAddExpense, onDelete }) => {
  const [selectedDate, setSelectedDate] = useState<string | null>(null);

  useEffect(() => {
    if (visible && initialDate) setSelectedDate(initialDate);
    else if (!visible) setSelectedDate(null);
  }, [visible, initialDate]);
      ...
      <KeyboardAvoidingView style={styles.overlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.modalCard}>
          <View style={styles.header}>
            <Text style={styles.title}>今月の全明細カレンダー</Text>
