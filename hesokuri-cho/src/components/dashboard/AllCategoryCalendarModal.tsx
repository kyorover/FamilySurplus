        ...
        <View style={styles.modalCard}>
          ...
          <View style={styles.monthControl}>
            <View style={styles.controlGroup}>
              <TouchableOpacity onPress={() => handleYearChange(-1)} style={styles.monthBtn}>
                <Text style={styles.monthBtnText}>≪ 年</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => handleMonthChange(-1)} style={styles.monthBtn}>
                <Text style={styles.monthBtnText}>＜ 月</Text>
              </TouchableOpacity>
            </View>
          ...
          {isLoading ? (
            ...
            <View style={styles.calendarGrid}>
              ...
              {calendarDays.map((dateStr, idx) => {
                ...
                const dayTotal =
                  expensesByDate[dateStr]?.reduce((s, e) => s + e.amount, 0) || 0;
            ...
            <View style={styles.detailSection}>
              <View style={styles.detailHeader}>
                <Text style={styles.detailDateText}>
                  {month}月{parseInt(selectedDate.split('-')[2], 10)}日の記録
                </Text>
                <TouchableOpacity
                  style={styles.addDirectBtn}
                  onPress={() => onAddExpense(selectedDate)}
                >
              ...
              <ScrollView style={styles.list} contentContainerStyle={styles.listContent}>
                {selectedExpenses.length === 0 ? (
                  ...
                  selectedExpenses.map((exp) => {
                    const catName =
                      categories.find((c) => c.id === exp.categoryId)?.name || '不明';
                    return (
                      <TouchableOpacity
                        key={exp.id}
